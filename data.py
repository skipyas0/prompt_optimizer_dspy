...
class Example:
    ...
    def qa_dict(self):
        if self.gold is None or isinstance(self.gold, tuple):
...
class Data:
    def __init__(self, data: list[dict] | dict[str, list[Example]], grading_function, eval_function, answer_type: type = str):
        if isinstance(data, list) and len(data) > 0 and isinstance(data[0], dict):
            if "answer" in data[0]:
                self.data = [Example(d["question"], gold=d["answer"], id=d["id"]) for d in data]
            else: 
                self.data = [Example(d["question"], gold=(d["test_inputs"], d["test_outputs"], d["code"]), id=d["id"]) for d in data]
            ...
            raise ValueError("Data must be a list of dictionaries or a tuple of three lists of examples.")
    ...
    def to_dict(self):
        return {
            ...
            "metadata": {
                "grading_function": self.grading_function.__name__ if self.grading_function else None,
                "eval_function": self.eval_function.__name__ if self.eval_function else None,
    ...
    def __get_solve_handle(self):
        ...
        def solve(question):
            try:
                ret = model.chain_of_thought(solve_sig, temp=SOLVE_TEMP, max_tries=1, task=question)
                return ret
    ...
    def from_attempts_file(cls, path):
        ...
        splits = {
            s: [Example.from_json(e) for e in data[s]]
    ...
    def eval_on_batch(
        ...
        if not prompt.valid:
            ...
            return 0.0
        grading_function = self.grading_function if phase == "optim" else self.eval_function
        if grading_function is not None and prompt.get_score("dev") > -1.0 and phase=="optim":
            logger.warning(f"Tried grading prompt {prompt.text} which is already graded.")
        ...
        for i, example in enumerate(batch):
            ...
            print(f"Added attempt {attempt.id} to example {example.id} and prompt {prompt.id}")
            print(f"Example {example.id} has {len(example.attempts)} attempts and prompt {prompt.id} has {len(prompt.attempts)} attempts")
            self.dump()
        if batch_score is not None:
            prompt.set_score(
