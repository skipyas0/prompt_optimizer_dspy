...
class Data:
    ...
    def __get_solve_handle(self):
        ...
        solve_sig = sig.Signature.from_str(f"task: str (task to be solved) -> solution: {str(self.answer_type)} ()")
        solve_module = lambda kwargs: model.chain_of_thought(solve_sig, temp=SOLVE_TEMP, **kwargs)
        def solve(question):
            try:
